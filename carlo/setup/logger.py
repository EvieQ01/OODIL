...
class CompleteLogger:
    ...
    def __init__(self, root, phase='train', use_time=False):
        self.root = root
        self.phase = phase
        ...
        if use_time:
            self.checkpoint_directory = os.path.join(self.root, "checkpoints", self.create_time)
        else:
            self.checkpoint_directory = os.path.join(self.root, "checkpoints")
        self.weight_directory = os.path.join(self.root, "weight_data")
        self.epoch = 0
        ...
        self.args_filename = os.path.join(self.root, "{}-{}-args.json".format(phase, now))
    ...
    def get_checkpoint_path(self, name=None):
        ...
        name = str(name)
        os.makedirs(os.path.join(self.checkpoint_directory, name), exist_ok=True)
        return os.path.join(self.checkpoint_directory, name + ".pth")

    def close(self):
        self.logger.close()
    def get_args_file(self):
        f = open(self.args_filename, 'w')
        return f