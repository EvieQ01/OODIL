...
class DCN(nn.Module):
    ...
    def __init__(self, args, device='cuda'):
        ...
        self.args = args
        self.lamda = args.lamda  # coefficient of the clustering term
        self.device = device
...
def featurize_with_dcn(demos_all, model:nn.Module, mode='c_n', sample_count=1, device='cuda',stride=20):
    '''
    ...
    for demo in demos_all:
        test_traj_segment = np.zeros((steps // stride, demo.shape[-1]))
        ...
        test_traj_segment = torch.from_numpy(test_traj_segment).to(device)
        # forward
        with torch.no_grad():
...
class traj_segment_generator():
    ...
    def get_downsample(self, begin_index=None)-> np.ndarray: 
        ...
        if begin_index is not None:
            assert begin_index < self.stride
        else:
    ...
    def get_downsample_all(self)-> np.ndarray: 
        ...
        for begin_index in range(1, self.stride):
...
class preDataset():
    ...
    def __init__(self, trajs, stride=20) -> None:
        ...
        for traj in trajs:
            ...
            downsample_all = np.expand_dims(traj_segment_generator(traj=traj, stride=stride, max_len=steps // stride).get_downsample_all(), axis=0)
    ...
    def get_paired_batch(self, batch_size=128):
        # traj_sample_index = np.random.randint(self.size, size=batch_size) # select [batch_size]
